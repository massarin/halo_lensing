...
def sigma_crit(z, zs, cosmo, comoving=False):
    ...
    surf_crit = 1.0e6 * (const.c**2 / (4.0 * np.pi * const.G) / (const.M_sun / const.pc)) * (dos / (dol * dls))
...
def nfw_bsigma(r, m, c, z, cosmo, mdef='vir', comoving=False):
    return nfw_sigma(r, m, c, z, cosmo, mdef, comoving) + nfw_dsigma(r, m, c, z, cosmo, mdef, comoving)
...
def tj_sigma_dl(x, c):
    ...
    m = (x > 1.0 + 1e-4) & (x < c)
    f[m] = ((-1.0) * np.sqrt(c*c - x[m]**2) / ((1.0 - x[m]**2) * (1.0 + c))
            - np.arccos((x[m]**2 + c) / (x[m] * (1.0 + c))) / ((x[m]**2 - 1.0) * np.sqrt(x[m]**2 - 1.0)))
    ...
    m = x < 1.0 - 1e-4
    f[m] = ((-1.0) * np.sqrt(c*c - x[m]**2) / ((1.0 - x[m]**2) * (1.0 + c))
            + np.arccosh((x[m]**2 + c) / (x[m] * (1.0 + c))) / ((1.0 - x[m]**2) * np.sqrt(1.0 - x[m]**2)))
...
def tj_bsigma_dl(x, c):
    ...
    m = (x > 1.0 + 1e-4) & (x < c)
    f[m] = ((np.sqrt(c*c - x[m]**2) - c) / (x[m]**2 * (1.0 + c))
            + np.log(x[m] * (1.0 + c) / (c + np.sqrt(c*c - x[m]**2))) / x[m]**2
            + np.arccos((x[m]**2 + c) / (x[m] * (1.0 + c))) / (x[m]**2 * np.sqrt(x[m]**2 - 1.0)))

    m = (x >= 1.0 - 1e-4) & (x <= 1.0 + 1e-4)
    f[m] = ((2.0*np.sqrt(c*c - 1.0) - c) / (1.0 + c)
            + np.log((1.0 + c) / (c + np.sqrt(c*c - 1.0))))

    m = x < 1.0 - 1e-4
    f[m] = ((np.sqrt(c*c - x[m]**2) - c) / (x[m]**2 * (1.0 + c))
            + np.log(x[m] * (1.0 + c) / (c + np.sqrt(c*c - x[m]**2))) / x[m]**2
            + np.arccosh((x[m]**2 + c) / (x[m] * (1.0 + c))) / (x[m]**2 * np.sqrt(1.0 - x[m]**2)))
...
def _f_dl_bmo(x):
    ...
    f1[m2] = (2.0*np.arctanh(a2) / np.sqrt(1.0 - x[m2]**2) - 1.0) / (1.0 - x[m2]**2)
...
def bmo_sigma_dl(x, t):
    ...
    f2 = (2.0*(t**2 + 1.0)*ff1 + 8.0*ff2
          + (t**4 - 1.0) / (t**2 * (t**2 + x**2))
          - np.pi * (4.0*(t**2 + x**2) + t**2 + 1.0) / ((t**2 + x**2) * np.sqrt(t**2 + x**2))
          + (t**2*(t**4 - 1.0) + (t**2 + x**2)*(3.0*t**4 - 6.0*t**2 - 1.0))
            * _l_dl_bmo(x, t) / (t**3 * (t**2 + x**2) * np.sqrt(t**2 + x**2)))
    return f1 * f2

def bmo_bsigma_dl(x, t):
    ff1, ff2 = _f_dl_bmo(x)
    f1 = t**4 / (2.0 * (t**2 + 1.0)**3 * x**2)
    f2 = (2.0*(t**2 + 4.0*x**2 - 3.0)*ff2
          + (1.0/t) * (np.pi*(3.0*t**2 - 1.0) + 2.0*t*(t**2 - 3.0)*np.log(t))
          + (1.0 / (t**3 * np.sqrt(t**2 + x**2)))
            * ((-1.0)*t**3*np.pi*(4.0*x**2 + 3.0*t**2 - 1.0)
               + (2.0*t**4*(t**2 - 3.0) + x**2*(3.0*t**4 - 6.0*t**2 - 1.0)) * _l_dl_bmo(x, t)))
    return f1 * f2
...
def m_bmo(x, t):
    ...
    f2 = ((t**2 + 1.0)*x*(x*(x + 1.0) - t**2*(x - 1.0)*(2.0 + 3.0*x) - 2.0*t**4)
          + t*(x + 1.0)*(t**2 + x**2)
...
def m_bmo_tot(t):
    ...
    f2 = (3.0*t**2 - 1.0) * (np.pi*t - t**2 - 1.0) + 2.0*t**2*(t**2 - 3.0)*np.log(t)
    return f1 * f2
...
def y_bmo(x, c, t):
    ...
    f2 = (2.0*(3.0*t**4 - 6.0*t**2 - 1.0)*p - 2.0*t*(t**4 - 1.0)*x*q
...
def kappa_off_2h_fft(r, m, c, f_cen, sig_off, tv, z, zs, flag_d, flag_out, cosmo, mdef='vir', comoving=False):
    return sigma_off_2h_fft(r, m, c, f_cen, sig_off, tv, z, flag_d, flag_out, cosmo, mdef, comoving) * inv_sigma_crit(z, zs, cosmo, comoving)
...
def sigma_off(r, m, z, f_cen, sig_off, flag_d, flag_out, cosmo, mdef='vir', comoving=False):
    ...
    if flag_out <= 1:
        s1h = bmo_sigma(r, m, c, tv, z, cosmo, mdef, comoving) if flag_d == 0 \
...
def kappa_off(r, m, z, f_cen, sig_off, zs, flag_d, flag_out, cosmo, mdef='vir', comoving=False):
